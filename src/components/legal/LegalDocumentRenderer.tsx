...
export const LegalDocumentRenderer: React.FC<LegalDocumentRendererProps> = ({ content }) => {
    const components = {
        h1: ({ node, ...props }: any) => (
            <h1 className="hidden" {...props} />
        ),
        h2: ({ node, ...props }: any) => (
            <h2 className="text-2xl font-semibold text-primary mt-10 mb-4" {...props} />
        ),
        h3: ({ node, ...props }: any) => (
        ...
        ul: ({ node, ...props }: any) => (
            <ul className="list-disc list-outside ml-6 mb-4 text-muted-foreground space-y-2" {...props} />
        ),
        ol: ({ node, ...props }: any) => (
            <ol className="list-decimal list-outside ml-6 mb-4 text-muted-foreground space-y-2" {...props} />
        ),
        li: ({ node, ...props }: any) => (
